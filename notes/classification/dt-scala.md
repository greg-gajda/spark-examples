...
```scala
def local(appName: String): SparkConf = {
...
```scala
def cluster(appName: String): SparkConf = {
    val conf = new SparkConf().setAppName(appName)
...
Functions to load data from local storage, HDFS and Cassandra go as follow. To run on cluster, local file should be located on network storage available for every Spark’s workers or copied to exactly the same location on every node. File name was mentioned before: bike-buyers.txt.
```scala
